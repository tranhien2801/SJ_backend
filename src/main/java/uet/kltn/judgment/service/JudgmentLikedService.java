...
@NoArgsConstructor
@Service
public class JudgmentLikedService {
    @Autowired
    ...
    public PageDto getJudgmentsLiked(ExpressionDto expressionDto, String userUid, FilterJudgmentRequestDto filterJudgmentRequestDto) {
        try {
            User user = userRepository.findByUid(userUid);
            if (user == null)   return null;
            Page<JudgmentLiked> judgmentPage;

            if (filterJudgmentRequestDto == null || filterJudgmentRequestDto.isEmpty()) {
                judgmentPage = judgmentLikedRepository.findAllByUserAndStateOrderByCreatedDesc(expressionDto.getPageable(), user, State.ACTIVE.getId());
            } else {
                ...
                if (filterJudgmentRequestDto.getPrecedent() == null) {
                    judgmentPage = judgmentLikedRepository.findByFilterAndState(
                    ...
                    judgmentPage = judgmentLikedRepository.findByFilterAndStateAndPrecedent(
                            expressionDto.getPageable(),
                            userUid,
                            filterJudgmentRequestDto.getJudgmentNumber() != null ? filterJudgmentRequestDto.getJudgmentNumber() : "",
                            filterJudgmentRequestDto.getCourtLevel() != null ? filterJudgmentRequestDto.getCourtLevel() : "",
